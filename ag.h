...
float achaMelhor(float *avaliacoes){
    int m = 0;
...
int achaPosicaoMelhor(float *avaliacoes){
    int m = 0;
    for(int i=1;i<TAMPOPULACAO;i++){
            if(avaliacoes[i]<avaliacoes[m])
            m = i;
...
float calculaMedia(float *avaliacoes){
    float valor = 0;
    for(int i=0;i<TAMPOPULACAO;i++)
        valor = valor + avaliacoes[i];
    valor = valor/TAMPOPULACAO;
    return(valor);
}
